...
def create_mask_from_user_drawing(img, draw=True):
    if draw == False:
    ...
    def draw_shape(event, x, y, flags, param):
        nonlocal ix, iy, drawing, mask_points

        # 当按下左键是返回起始位置坐标
        if event == cv2.EVENT_LBUTTONDOWN:
            drawing = True
            ix, iy = x, y
            mask_points = [(x, y)]  # 重新开始画，记录第一个点
...
def generate_frames(video_file):
    ...
    first_frame = cv2.VideoCapture(video_file).read()[1]
    first_frame = cv2.resize(first_frame, (w, h))
    mask = create_mask_from_user_drawing(first_frame)
    ...
    print(f'Finish sampling frames from {video_file}. resolution: {w}x{h}, fps: {fps}')
    return frame_list, fps, (w, h)

def generate_frames_range(video_file, mask, start_frame, f_range):
    """ Sample frames from the video.

        Args:
            video_file (str): File path of the video file
            mask (numpy.ndarray): Mask to apply on the frames
            start_frame (int): Starting frame index
            range (int): Number of frames to sample
    ...
    # 逐帧读取视频
    # for frame_number in range(start_frame):
    #     ret, frame = cap.read()
    
    cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame)
        
    count = 0
    while success and count < f_range:
        # print current frame
    ...
    print(f'Finish sampling frames from {video_file}. resolution: {w}x{h}, fps: {fps}', f'from {start_frame} to {start_frame+f_range}')
    return frame_list, fps, (w, h)
